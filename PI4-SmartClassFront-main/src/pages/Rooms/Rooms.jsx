...
const Rooms = () => {
  ...
  const [formRegra, setFormRegra] = useState({ usuarioId: '', salaId: '', inicio: '', fim: '', dias: [] });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (resRooms.status === 401) { localStorage.removeItem('token'); navigate('/login'); return; }
  ...
  const handleAddSala = async (e) => {
    ...
    try {
      ...
      if (response.ok) { setNovaSala(''); fetchData(); alert("Sala cadastrada com sucesso!"); }
    } catch (err) { console.error(err); }
  };

  const handleSaveRegra = async (e) => {
    e.preventDefault();
    if (formRegra.dias.length === 0) { alert("Por favor, selecione pelo menos um dia da semana."); return; }
    const token = localStorage.getItem('token');
    ...
    try {
      const response = await fetch(url, {
        ...
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({
          user_id: formRegra.usuarioId, room_id: formRegra.salaId,
          allowed_start: formRegra.inicio, allowed_end: formRegra.fim,
          allowed_days: formRegra.dias.join(',')
        })
      });
      if (response.ok) {
  ...
  const handleEditClick = (regra) => {
    ...
    setFormRegra({
      usuarioId: regra.user_id, salaId: regra.room_id,
      inicio: regra.allowed_start, fim: regra.allowed_end,
      dias: regra.allowed_days ? regra.allowed_days.split(',') : []
    });
  ...
  const handleDeleteRule = async (id) => {
    ...
    try {
      ...
      if (response.ok) setRegras(prev => prev.filter(r => r.id !== id));
    } catch (err) { console.error(err); }
  };

  const filteredRegras = regras.filter((regra) => {
    const term = searchTerm.toLowerCase();
    return regra.user_name.toLowerCase().includes(term) || regra.room_name.toLowerCase().includes(term);
  });
    ...
    return (
      <div className="flex gap-1 justify-center flex-wrap max-w-xs mx-auto">
        {DAYS_OF_WEEK.map(d => {
          ...
          return (
            <span key={d.id}
              className={`text-[10px] font-extrabold px-1.5 py-0.5 rounded-md transition-colors ${isAllowed ? 'bg-senac-orange text-white shadow-sm' : 'bg-gray-100 text-gray-300'}`}>
              {d.label}
            </span>
          );
        })}
  ...
  return (
    <PageLayout>
      <header className="mb-6 md:mb-10">
        <h1 className="text-2xl md:text-3xl font-bold text-senac-blue">Salas e Horários</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8 mb-8">
        {isGestor && (
          <div className="lg:col-span-1">
            <section className="bg-senac-card p-4 md:p-6 rounded-xl border border-gray-200 shadow-sm lg:sticky lg:top-6">
              <h2 className="text-base md:text-lg font-bold text-senac-blue mb-4">Cadastrar Nova Sala</h2>
              <form onSubmit={handleAddSala} className="flex flex-col gap-4">
                <input type="text" value={novaSala} onChange={(e) => setNovaSala(e.target.value)}
        ...
        <div className={`${isGestor ? 'lg:col-span-2' : 'lg:col-span-3'} flex flex-col gap-6 md:gap-8`}>
          {isGestor && (
            <section className={`bg-senac-card p-4 md:p-6 rounded-xl border shadow-sm transition-all ${editingRuleId ? 'border-senac-orange ring-1 ring-senac-orange/30 bg-orange-50/10' : 'border-gray-200'}`}>
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-base md:text-lg font-bold text-senac-blue">
              ...
              <form onSubmit={handleSaveRegra} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-senac-text uppercase tracking-wide">Usuário</label>
                  <select name="usuarioId" value={formRegra.usuarioId} onChange={handleFormRegraChange} required
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <select name="salaId" value={formRegra.salaId} onChange={handleFormRegraChange} required
                    className="p-3 rounded-lg border border-gray-300 bg-white text-sm focus:outline-none focus:border-senac-orange text-senac-text">
                    <option value="">Selecione...</option>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <input type="time" name="inicio" value={formRegra.inicio} onChange={handleFormRegraChange} required
                    className="p-3 rounded-lg border border-gray-300 bg-white text-sm focus:outline-none focus:border-senac-orange text-senac-text" />
                </div>

                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-senac-text uppercase tracking-wide">Fim (HH:MM)</label>
                  <input type="time" name="fim" value={formRegra.fim} onChange={handleFormRegraChange} required
                    className="p-3 rounded-lg border border-gray-300 bg-white text-sm focus:outline-none focus:border-senac-orange text-senac-text" />
                </div>
                  ...
                  <div className="flex flex-wrap gap-3 p-3 rounded-lg border border-gray-300 bg-white">
                    {DAYS_OF_WEEK.map(day => (
                      <label key={day.id} className="flex items-center gap-2 text-sm text-senac-text font-medium cursor-pointer select-none">
                        <input type="checkbox" checked={formRegra.dias.includes(day.id)} onChange={() => handleDayCheckboxChange(day.id)}
                ...
                <div className="sm:col-span-2 flex justify-end mt-2">
                  <button type="submit"
                    className={`text-white px-6 py-3 rounded-lg font-bold text-sm transition-colors shadow-md ${editingRuleId ? 'bg-green-600 hover:bg-green-700' : 'bg-senac-orange hover:bg-senac-orange-hover'}`}>
          ...
          <section className="bg-senac-card p-4 md:p-6 rounded-xl border border-gray-200 shadow-sm">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 border-b border-gray-100 pb-4 gap-4">
              <h2 className="text-base md:text-lg font-bold text-senac-blue">Regras de Uso Ativas</h2>
              <div className="relative w-full sm:w-64">
                <input type="text" placeholder="Buscar usuário ou sala..."
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-gray-200 text-xs font-bold text-senac-muted uppercase tracking-wider">
                      ...
                      <th className="pb-3 hidden sm:table-cell">Ambiente</th>
                      <th className="pb-3 text-center hidden md:table-cell">Dias</th>
                    ...
                    {filteredRegras.map(regra => (
                      <tr key={regra.id} className="hover:bg-gray-50 transition-colors">
                        <td className="py-3 font-semibold">
                          {regra.user_name}
                          <div className="text-xs text-senac-muted sm:hidden">{regra.room_name}</div>
                        </td>
                        <td className="py-3 text-senac-muted hidden sm:table-cell">{regra.room_name}</td>
                        <td className="py-3 text-center hidden md:table-cell">{renderDaysBadges(regra.allowed_days)}</td>
                        <td className="py-3 text-center">
                          <span className="bg-blue-50 text-senac-blue font-bold px-2 md:px-3 py-1 rounded-full text-xs">
                        ...
                        {isGestor && (
                          <td className="py-3 text-right">
                            <div className="flex justify-end gap-2">
                              <button onClick={() => handleEditClick(regra)}
                                className="text-senac-blue bg-blue-50 hover:bg-blue-100 p-2 rounded text-xs font-bold transition-colors" title="Editar">✏️</button>
                              <button onClick={() => handleDeleteRule(regra.id)}
                                className="text-red-600 bg-red-50 hover:bg-red-100 p-2 rounded text-xs font-bold transition-colors" title="Excluir">🗑️</button>
                            </div>
                          </td>
                        )}
                      </tr>
